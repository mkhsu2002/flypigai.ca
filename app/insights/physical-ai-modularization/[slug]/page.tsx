...
export default async function SeriesArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main>
    ...
    <article className="article-shell shell">
      <header className="article-header"><p className="eyebrow">Physical AI Goes Modular · Essay {article.number}</p><h1>{article.title}</h1><p className="article-deck">{article.description}</p><div className="article-meta"><span>By FlyPig AI Insights</span><span>Published September 5, 2026</span>{review ? <span>Updated {formatEvidenceDate(review.modifiedDate)}</span> : null}<span>Research series</span></div></header>
