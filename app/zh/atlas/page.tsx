...
export default function ZhAtlasPage() {
  return <main lang="zh-Hant">
    ...
    <section className="atlas-hero shell">
      <p className="eyebrow">加拿大機器人產業生態系</p>
      <h1>Canada Physical AI Atlas</h1>
      <p className="lead">持續更新的加拿大機器人、無人機、自主系統、關鍵技術、系統整合、研究機構與產業需求地圖。</p>
      <div className="atlas-stats"><div><strong>{atlasCategories.length}</strong><span>個生態系分類</span></div><div><strong>{atlasCategories.reduce((sum, category) => sum + category.companies.length, 0)}</strong><span>家首批收錄機構</span></div><div><strong>Canada</strong><span>加拿大市場焦點</span></div></div>
    </section>

    <section className="section shell">
      <div className="section-head"><div><p className="eyebrow">探索產業生態系</p><h2>從關鍵技術，一路連接到真實產業部署。</h2></div><p className="section-copy">Atlas 依照不同機構在市場中的角色分類，包括技術研發、平台製造、系統整合、研究與產業採用。每個分類先收錄 6 家代表性機構，後續將持續擴充。</p></div>
    ...
    <section className="cta shell"><div className="cta-box"><div><p className="eyebrow">加入或合作</p><h2>你的公司是否也應該被收錄在 Atlas？</h2></div><div className="actions"><a className="pill primary" href="/zh/contact">聯絡我們</a></div></div></section>
