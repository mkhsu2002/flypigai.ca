...
export default function ZhHomePage() {
  return <main lang="zh-Hant">
    ...
    <section className="section dark"><div className="shell"><div className="section-head"><div><p className="eyebrow">研究範圍</p><h2>聚焦智慧硬體背後的關鍵技術層。</h2></div><p className="section-copy">我們優先整理會影響產品架構、開發風險、供應鏈與部署可行性的技術，讓每一項公開內容都能支援實際判斷。</p></div><div className="grid3">{domains.map((domain, index) => <article className="card" key={domain}><span className="num">{String(index + 1).padStart(2, "0")}</span><h3>{domain}</h3></article>)}</div></div></section>
    <section className="section shell"><div className="section-head"><div><p className="eyebrow">合作方式</p><h2>從公開情報走向有邊界的商業決策。</h2></div><p className="section-copy">加拿大產品團隊可評估台灣技術路徑；台灣科技業者可準備加拿大應用與 Design-In 對話。兩條路徑都先交付可判斷的簡報，再評估是否需要技術引介。</p></div><div className="grid2"><article className="card"><span className="num">加拿大產品團隊</span><h3>Edge AI 技術路徑與資格評估</h3><p>釐清需求、比較架構選項，並依適配、風險與待驗證問題評估台灣技術候選。</p><div className="actions"><a className="pill secondary" href="/services/canadian-product-teams" lang="en">查看合作範圍</a></div></article><article className="card"><span className="num">台灣科技業者</span><h3>加拿大應用與 Design-In 準備度</h3><p>把產品能力轉譯成加拿大應用情境，並辨識可信技術對話前需要補齊的證據。</p><div className="actions"><a className="pill secondary" href="/zh/services/taiwan-technology-companies">查看合作範圍</a></div></article></div></section>
