...
function wrap(text, max = 42) { const words = String(text).split(/\s+/); const lines = []; let line = ""; for (const word of words) { const next = line ? `${line} ${word}` : word; if (next.length <= max) line = next; else { if (line) lines.push(line); line = word; } } if (line) lines.push(line); return lines.slice(0, 4); }
function svg(signal) {
  ...
  const cards = facts.slice(0, 3).map((fact, index) => { const x = 110 + index * 460; const factLines = wrap(fact, 22).slice(0, 3); let fy = cardY + 116; const texts = factLines.map((line) => { const el = `<text x="${x + 34}" y="${fy}" font-family="Inter,Arial,sans-serif" font-size="27" font-weight="600" fill="#1f2933">${escapeXml(line)}</text>`; fy += 38; return el; }).join(""); return `<rect x="${x}" y="${cardY}" width="400" height="220" rx="24" fill="#fff" stroke="#d7e1df"/><text x="${x + 34}" y="${cardY + 56}" font-family="Inter,Arial,sans-serif" font-size="18" letter-spacing="2" fill="#0f766e">${labels[index]}</text>${texts}`; }).join("");
